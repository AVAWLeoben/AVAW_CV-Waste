...
class UserInterface:
    ...
    def create_gui(self):
        title = "Mustertitel" #* Path(image_paths[0]).stem
       ...
       # Canvas erstellen
        ...
        dropdown = ttk.Combobox(buttons_frame, textvariable=selected, values=self.owner.class_names, state="readonly")
        dropdown.grid(row=2, column=1, padx=5, pady=5)
        # Delay setting the current selection to ensure Combobox is fully loaded
        dropdown.after(100, lambda: dropdown.set("Select your Class..."))
...
class MenuBar:
    ...
    def creat_menu(self):
        ...
        # file_menu.add_command(label="Change Classes", command=load_new_class_name_list)
