...
class UserInterface:
    def __init__(self, owner):
        ...
        self.tk_img = None
        self.create_gui()
        self.owner.style = ttk.Style()
        # print(self.owner.style.theme_names())  # List available themes
        self.owner.style.theme_use('xpnative')
    ...
    def create_gui(self):
       ...
       # Canvas erstellen
        ...
        def update_threshold_label(event=None):
            # Optional: round to desired step
            step = 0.1
            value = round(self.owner.threshold.get() / step) * step
            self.owner.threshold.set(value)
            slider_value.config(text=f"{value:.1f}")
            
        def threshold_on_click(event):
            # Handle clicks on the trough - convert pixel position to value
            scale_width = slider.winfo_width()
            if scale_width > 0:
                # Calculate the value based on click position
                fraction = event.x / scale_width
                fraction = max(0.0, min(1.0, fraction))  # Clamp between 0 and 1
                new_value = 0.0 + fraction * (1.0 - 0.0)  # from_=0.0, to=1.0
                self.owner.threshold.set(new_value)
                update_threshold_label()
            
            # Start tracking drag motion
            slider.bind("<B1-Motion>", threshold_on_drag)
            return "break"
        
        def threshold_on_drag(event):
            # Handle dragging - same logic as click
            scale_width = slider.winfo_width()
            if scale_width > 0:
                fraction = event.x / scale_width
                fraction = max(0.0, min(1.0, fraction))
                new_value = 0.0 + fraction * (1.0 - 0.0)
                self.owner.threshold.set(new_value)
                update_threshold_label()
            return "break"
        
        self.owner.threshold = tk.DoubleVar(value=0.0)
        
        slider = ttk.Scale(buttons_frame, from_=0.0, to=1.0, orient='horizontal', variable=self.owner.threshold)
        slider.grid(row=0, column=1, padx=5, pady=5)
        
        slider.bind("<Button-1>", threshold_on_click)
        slider.bind("<ButtonRelease-1>", lambda e: slider.unbind("<B1-Motion>"))
        slider.bind("<ButtonRelease-1>", update_threshold_label)
        slider.bind("<B1-Motion>", update_threshold_label)
        
        slider_value = tk.Label(buttons_frame, text=f"{self.owner.threshold.get():.1f}")
        slider_value.grid(row=0, column=2, padx=5, pady=5, sticky="w")
        
        anno_button = ttk.Button(buttons_frame, text="Annotate Image (space)")#* command=lambda: start_seg_app(bu_root))
        anno_button.grid(row=1, column=0, padx=5, pady=5)
        
        simply_button = ttk.Button(buttons_frame, text="Simplify Image (B)")#* command=lambda: start_seg_app(bu_root))
        simply_button.grid(row=1, column=1, padx=5, pady=5)
        
        save_button = ttk.Button(buttons_frame, text="Save Annotation (S)")#* command=lambda: start_seg_app(bu_root))
        ...
        prev_button = ttk.Button(buttons_frame, text="Previous Image (<-)", command=self.owner.IMG_HANDLER.previous_image)
        prev_button.grid(row=3, column=0, padx=5, pady=5)
        
        
        jump_button = ttk.Button(buttons_frame, text="Jump to Image (Enter)", command=self.owner.IMG_HANDLER.jump_to_image)
        jump_button.grid(row=3, column=1, padx=5, pady=5)
        
        next_button = ttk.Button(buttons_frame, text="Next Image (->)", command=self.owner.IMG_HANDLER.next_image)
        next_button.grid(row=3, column=2, padx=5, pady=5)
        
        img_number = ttk.Entry(buttons_frame, textvariable=self.owner.number, justify="center")
        img_number.grid(row=4, column=1, padx=5, pady=5)
        
        undo_button = ttk.Button(buttons_frame, text="Undo (Z)")#* command=lambda: start_seg_app(bu_root))
...
class MenuBar:
    ...
    def creat_menu(self):
        ...
        global_menu.add_command(label="Alter Class Names", command=self.owner.CLASS_HANDLER.edit_class_names_window)
...
class SegmentationHandler:
    ...
    def segment_image_with_morphhelper(event=None):
        global annotations
        self.owner.annotations = self.owner.SEGHELPER.segment_image(self.owner.image_paths[self.owner.curr_image_index])
        self.owner.update_display(None)
...
class AnnotationHandler:
    ...
    # def add_annotations(self, event=None):
    ...
    # def copy_annotations(self, event=None):
    #     global annotations, last_annotations
    #     last_annotations = copy.deepcopy(annotations)
    #     update_display()

    # def paste_annotations(self, event=None):
    #     global annotations, last_annotations
    #     if last_annotations is None:
    #         tk.messagebox.showinfo("showinfo", "Nothing To Paste") 
    #         return
    ...
    # def copy_annotation(self, event=None):
    ...
    # def merge_annotations(self, event=None):
    #     global annotations, copied_annotation, selected_mask_idx, multiselect_on
    ...
    # def paste_annotation(self, event=None):
...
class ClassHandler:
    ...
    def edit_class_names_window(self):
        def save_changes():
            ...
            if input_text:
                self.owner.class_names[:] = [name.strip() for name in input_text.split(',') if name.strip()]
        ...
        text_input.insert(tk.END, ", ".join(self.owner.class_names))  # Show current class names

        save_button = ttk.Button(window, text="Save", command=save_changes)
...
class SEG_App:
    def __init__(self,caller_root=None):        
        ...
        self.SEGHELPER = MorphologicSegHelper()
        self.CLASS_HANDLER = ClassHandler(self)
        self.CLASS_HANDLER.load_class_names()
        
        self.IMG_HANDLER = ImageHandler(self)
        self.SEG_HANDLER = SegmentationHandler(self)
        self.ANNO_HANDLER = AnnotationHandler(self)
