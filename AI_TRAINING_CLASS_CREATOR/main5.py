...
def train_model(caller=None):   
    ...
    if model_of_choice == "Classifier":
        ...
        p = multiprocessing.Process(target=train_classifier,args=(TRAINING_FLAG,))
        p.start()
    else:
        easygui.msgbox(f"{model_of_choice} not yet implemented")

def train_classifier(training_flag):
    from ultralytics import YOLO
    import os
    import easygui
    ...
    model.train(data=train_folder, epochs=n_epochs, imgsz=imgsz)
...
def draw():
    ...
    if TRAINING_FLAG.value:
        # Show "Training in Progress..." message or animation
