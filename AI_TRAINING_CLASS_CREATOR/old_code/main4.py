...
def train_classifier():
    ...
    progress_bar = ProgressBar(100, 300, 600, 40, "Training Preparation")
    progress = 0.0
    running = True
    ...
    draw()
    train_folder = easygui.diropenbox("Select YOLO classification folder (train/test)")
    if not train_folder:
        return
    ...
    progress_bar.update(0.7)
    draw()

    # === Step 5: Train Model ===
    progress_bar.title = "Training YOLO Model..."
    draw()
...
def train_classifier2():
    ...
    train_folder = easygui.diropenbox("Select folder with YOLO classification data (train/test folders)")
    if not train_folder:
        return
    ...
    # Train the model
    results = model.train(data=train_folder, epochs=n_epochs, imgsz=imgsz)
