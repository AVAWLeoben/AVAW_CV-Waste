...
def load_model(model_path):
    ...
    Falls back to YOLO('yolov8n-seg.pt') if no model is found.
    """
    error_messages = []
    ...
    # 4. If all failed, use placeholder YOLO model
    messagebox.showwarning(
        "Model Load Failed",
        f"Could not load a valid model from {model_path}.\n\n"
        + "\n".join(error_messages)
        + "\n\nLoading placeholder model: yolov8n-seg.pt"
    )

    try:
        # Download placeholder YOLO segmentation model
        model = YOLO("yolov8n-seg.pt")
        if is_valid_model(model):
            generate_colors(len(model.names))
            return model
    except Exception as e:
        messagebox.showerror("Fallback Failed", f"Failed to load placeholder model: {e}")
        return None
